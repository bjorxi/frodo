...
function npmInit(projectPath) {
  ...
  if (!$SKIP_VIEWS) {
    console.log('Running: npm install jade --save');
    exec('npm install jade --save');
  }
}
...
function generateController(name, methods) {
  ...
  generateRoutes(name, fullName, methods, $SCAFFOLD);
}
...
function getDefaultViewContent (viewPath) {
  return "extends ../layouts/application.jade\n\n" +
         "block content\n"+
